<!DOCTYPE html>
<html>
<head>
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="stylesheet" type="text/css" href="./css/azienda.css">
	<link href="https://fonts.googleapis.com/css?family=Poppins:300,400,500&display=swap" rel="stylesheet">
	<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
	<title>PC Home</title>
</head>
<body>
	<!-- navbar -->
	

	
	
	<!-- main pagina -->
	<section id="maintop">
		<header class="super-top">
		<h1>La tua azienda può essere qui!</h1>
		<h2>Contattaci per realizzare il sito WEB<br>
			perfetto per la tua azienda.
		</h2>
		<!-- <button href="#scegli">Scegli il tuo pc</button> -->
		<a href="index.php">Ritorna All'home</a>
		</header>
	</section>
	<section id="scegli">
		<header class="spacer">
			
		</header><!-- /header -->
	<section id="scegli">
		<header class="chisiamo">
		<h1>Chi siamo?</h1>	
			<h2>
			Scopri come può diventare il sito per il tuo bar<br>
				bassa, media e alta <br>
				scegli la fascia più adatta al te e guarda cosa possiamo offrire 
			</h2>
			<a href="#maintop" title="Tornasu"><img src="arrow-up.svg" alt="Torna su"></a>	
		</header><!-- /header -->
	
		<section id="scegli">
		<header class="spacer">
			
		</header><!-- /header -->


		<section id="scegli">
		<header class="dovesiamo">
		
			<h2>
			Ecco i nostri numeri per info e prenotazioni:<br>
				bassa, media e alta <br>
				scegli la fascia più adatta al te e guarda cosa possiamo offrire 
			</h2>
			<h1>Dove siamo?</h1>	
		</header><!-- /header -->
		<section id="scegli">
		<header class="spacer">
			
		</header><!-- /header -->	

	<section id="scegli">
		<header class="lavoraconnoi">
		<h1>Lavora con noi!</h1>	
			<h2>
			Ecco cosa possiamo offrirti <br>
				bassa, media e alta <br>
				scegli la fascia più adatta al te e guarda cosa possiamo offrire 
			</h2>
			
		</header><!-- /header -->
		<section id="scegli">
		<header class="spacer">
			
		</header><!-- /header -->	
		
	</body>
		</html>